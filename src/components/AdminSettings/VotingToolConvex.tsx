...
type VoteValue = "POSITIVE" | "NEGATIVE" | "NEUTRAL";
...
export interface VoteExtendedConvex {
  _id: string;
  value: VoteValue;
  testerId: string;
...
type VotesProps = {
  postDetailsId: string;
  testerId: string;
  votes: VoteExtendedConvex[];
  setVotes: (votes: VoteExtendedConvex[]) => void;
};

export const VotingToolConvex = (props: VotesProps) => {
  ...
  const setVotes = (value: VoteValue) => {
    const updated = props.votes.map((vote) =>
      vote.testerId === props.testerId ? { ...vote, value } : vote
    );
    props.setVotes(updated);
  };

  const testerVote = props.votes.find(
    (vote) => vote.testerId === props.testerId
  );
  ...
  return (
    <div class={"flex flex-col"}>
      <div class={"w-40 flex flex-row justify-between font-semibold"}>
        ...
        <div
          class={
            "flex flex-row gap-1 text-white text-[14px] transition-all duration-300"
          }
        >
          <button
            onClick={async () => {
            ...
            class={`w-7 py-1 bg-border-light border border-primary rounded ${
              testerVote && testerVote.value === "NEGATIVE"
                ? "bg-primary"
                : "hover:bg-primary"
            }`}
          >
          ...
          <button
            ...
            class={`w-7 py-1 bg-border-light border border-primary rounded ${
              testerVote && testerVote.value === "NEUTRAL"
                ? "bg-primary"
                : "hover:bg-primary"
            }`}
          >
          ...
          <button
            ...
            class={`w-7 py-1 bg-border-light border border-primary rounded ${
              testerVote && testerVote.value === "POSITIVE"
                ? "bg-primary"
                : "hover:bg-primary"
            }`}
          >
      ...
      <ul
        class={
          "flex flex-col gap-2 mt-2 font-semibold overflow-y-scroll light-scrollbar"
        }
      >
        {props.votes.map((vote) => (
          <li
            key={vote.testerId}
            class={
              "h-7 flex flex-row justify-between px-2 bg-dark darkborder rounded"
            }
          >
