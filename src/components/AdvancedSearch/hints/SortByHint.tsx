...
export const SortByHint: HintCreator = () => {
  const title = 'Posts sorting'

  const criteria = ['likes', 'comments']
  ...
  return {
    ...
    mapper: ({ value, onSelect }) => {
      ...
      const filterAuthors = (val?: string) => criteria.filter(a => (val ? a.startsWith(val) : a))
