...
const PostTitle = (props: PostTitleProps) => {
  return (
    <div className={'flex flex-row items-center text-white gap-[15px] font-semibold'}>
      {props.isEditMode ? (
        <select
          value={props.type}
        ...
        <h1 className={'bg-gray py-[9px] px-[15px] rounded-[3px] font-[Glory] text-[24px]'}>{props.type}</h1>
      )}
      {props.isEditMode ? (
        <input
          value={props.title}
        ...
        <a href={props.linkTo} className={'text-[28px]'}>
          {props.title}
        </a>
      )}
    </div>
